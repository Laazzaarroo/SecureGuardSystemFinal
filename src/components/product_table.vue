<template>
  ...
  <v-card flat title="All Products" class="text-cyan-darken-4">
    <template v-slot:append>
        <v-btn @click="openDialog" elevated prepend-icon="mdi-plus-box" class="ma-2 bg-cyan-darken-4">ADD Product</v-btn>
        <v-btn @click="openBrand" elevated prepend-icon="mdi-view-list" class="ma-2 bg-cyan-darken-4">Brands</v-btn>
    </template>
    ...
    <v-data-table :headers="headers" :items="cctvlist" :search="search" class="ma-5">
      ...
      <template v-slot:item.action ="{ item }">
        <v-btn @click="openDialog" variant="tonal" class="me-2 text-cyan-darken-4" prepend-icon="mdi-pencil">Edit</v-btn>
        <v-btn @click="unlist(item.id)" variant="elevated" prepend-icon="mdi-delete" class="bg-cyan-darken-4">Unlist</v-btn>
      </template>
    </v-data-table>
  </v-card>
  ...
  <v-dialog v-model="dialog" max-width="500">
      <v-card>
        <v-card-title class="text-cyan-darken-4">Add Products</v-card-title>
        <v-card-text>
          <v-form>
        ...
        <v-card-actions>
          <!-- BUTTONS -->
          <v-btn variant="tonal" class="mx-2 mb-2 d-flex text-cyan-darken-4"  @click="closeDialog">Cancel</v-btn>
          <v-btn variant="elevated" class="mx-2 mb-2 bg-cyan-darken-4"  @click="submitForm">Submit</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<v-dialog v-model="branddialog" max-width="1000">
      <v-card>
        <v-card-title class="text-cyan-darken-4">Manage Brands</v-card-title>
          <v-card-text>
          <v-form>
            <v-row>
            <v-text-field v-model="model" label="Add Brand Name"></v-text-field>
            <v-btn flat="true" size="x-large" class="bg-cyan-darken-4">ADD</v-btn>
            </v-row>
          <v-card-title class="text-cyan-darken-4 ma-0 pa-0">List of Available Brands</v-card-title>
        ...
        <v-card-actions>
          <!-- BUTTONS -->
          <v-btn variant="elevated" class="mx-2 mb-2 bg-cyan-darken-4" @click="closeBrand">Cancel</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
</template>
